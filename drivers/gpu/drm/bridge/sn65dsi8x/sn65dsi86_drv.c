...
unsigned int sn65dsi86_get_max_lanes(struct sn65dsi86_data *pdata)
{
	u8 data;
	int ret;
...
void sn65dsi86_bridge_enable(struct drm_bridge *bridge)
{
	...
	#ifdef GET_MAX_LANES_FROM_PANEL
	int max_dp_lanes;
	#endif
...
if (0) {
	...
	pdata->dp_lanes = min(pdata->dp_lanes_from_dts, max_dp_lanes);
	#else
	pdata->dp_lanes = pdata->dp_lanes_from_dts;
	#endif
				     ...
				     struct device_node *np)
{
	...
	pdata->dp_lanes_from_dts = dp_lanes;
