...
static int ili9881c_get_modes(struct drm_panel *panel)
{
	...
	mode = drm_mode_duplicate(panel->drm, &ili9881c_default_mode);
	if (!mode) {
		dev_err(&ctx->dsi->dev, "failed to add mode %ux%ux@%u\n",
			ili9881c_default_mode.hdisplay,
			ili9881c_default_mode.vdisplay,
			ili9881c_default_mode.vrefresh);
		return -ENOMEM;
	}
...
static int ili9881c_dsi_probe(struct mipi_dsi_device *dsi)
{
	...
	dsi->mode_flags = MIPI_DSI_MODE_VIDEO_BURST | MIPI_DSI_MODE_VIDEO |MIPI_DSI_MODE_LPM;
